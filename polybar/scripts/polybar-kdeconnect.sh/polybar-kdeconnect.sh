...
YOFFSET=30
XOFFSET=450
THEME=~/.config/polybar/scripts/polybar-kdeconnect.sh/kdeconnect

# Color Settings of Icon shown in Polybar
COLOR_DISCONNECTED='#002333'       # Device Disconnected
COLOR_NEWDEVICE='#747E7E'          # New Device
COLOR_BATTERY_90='#DEEFE7'         # Battery >= 90
COLOR_BATTERY_80='#159A9C'         # Battery >= 80
COLOR_BATTERY_70='#B4BEC9'         # Battery >= 70
COLOR_BATTERY_60='#3F7C85'         # Battery >= 60
COLOR_BATTERY_50='#00CCBF'         # Battery >= 50
COLOR_BATTERY_LOW='#FF5F5D'        # Battery <  50
...
show_menu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" -location $LOCATION -yoffset $YOFFSET -xoffset $XOFFSET -theme $THEME -line-padding 4 -padding 20 -lines 5 <<< "Battery: $DEV_BATTERY%|Ping|Find Device|Send File|Browse Files|Unpair")"
                case "$menu" in
...
show_pmenu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" -location $LOCATION -yoffset $YOFFSET -xoffset $XOFFSET -theme $THEME -line-padding 1 -padding 20 -lines 1<<<"Pair Device")"
                case "$menu" in
...
show_pmenu2 () {
    menu="$(rofi -sep "|" -dmenu -i -p "$1 has sent a pairing request" -location $LOCATION -yoffset $YOFFSET -xoffset $XOFFSET -theme $THEME -line-padding 4 -padding 20 -lines 2 <<< "Accept|Reject")"
                case "$menu" in
